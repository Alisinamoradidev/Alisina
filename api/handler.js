        ...
        <tr><td style="padding:24px 40px;border-top:1px solid #e2e8f0;text-align:center;color:#94a3b8;font-size:13px">
          ...
          if (gmailUser && gmailPass) {
            ...
            const sendOne = async (to, subject, html) => {
              ...
              try {
                const info = await t.sendMail({ from: `"Alisina Realty" <${gmailUser}>`, to, subject, html });
                console.log('email sent to', to, info.messageId);
              } catch (e) { console.error('email error to', to, e.message); }
            };
            if (toEmail) await sendOne(toEmail, `New payment received — ${propName}`, emailLayout('New Payment Received', `
