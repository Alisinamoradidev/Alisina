...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (path === '/payments/test-webhook-email' && method === 'POST') {
      const { testTo, testCustomer, testName, testProp, testAmount, testType, testReceipt } = body;
      const { data: gmailConfig } = await supabase.from('settings').select('value').eq('key', 'gmail_smtp').maybeSingle();
      const gmailUser = gmailConfig?.value?.email;
      const gmailPass = gmailConfig?.value?.appPassword;
      const toEmail = testTo, customerEmail = testCustomer, customerName = testName || 'Test', propName = testProp || 'Test Property';
      const amount = testAmount || 1000, type = testType || 'deposit', receiptUrl = testReceipt || 'https://dashboard.stripe.com';
      if (gmailUser && gmailPass) {
        const nodemailer = require('nodemailer');
        const t = nodemailer.createTransport({ host: 'smtp.gmail.com', port: 587, secure: false, auth: { user: gmailUser, pass: gmailPass } });
        const results = [];
        if (toEmail) {
          try {
            ...
            results.push({ to: toEmail, status: 'ok' });
          } catch (e) { results.push({ to: toEmail, status: 'error', message: e.message }); }
        }
        if (customerEmail && customerEmail !== toEmail) {
          try {
            await t.sendMail({
            ...
            results.push({ to: customerEmail, status: 'ok' });
          } catch (e) { results.push({ to: customerEmail, status: 'error', message: e.message }); }
        }
