          ...
          if (!gmailUser || !gmailPass) { console.error('Gmail not configured'); } else {
            ...
            await Promise.all([
              toEmail ? transporter.sendMail({
                ...
                `),
              }).then(() => { console.log('admin email ok'); }).catch(e => { console.error('admin email error:', e.message); }) : Promise.resolve(),
              customerEmail ? transporter.sendMail({
                ...
                `),
              }).then(() => { console.log('customer email ok'); }).catch(e => { console.error('customer email error:', e.message); }) : Promise.resolve(),
            ]);
          }
        } catch (e) {
